...
function add(item) {
  ...
  item._id = [...window.crypto.getRandomValues(randValues)].map(v => v.toString(16)).join('')

  itemsStore.update(state => sort([
    ...state,
    item,
  ]))
...
function update(item) {
  itemsStore.update(state => sort(state.map(i => i._id == item._id
    ? item
    : i)))
